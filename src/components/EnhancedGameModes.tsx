...
import { Zap, Heart, Clock, X, Sword, Skull, Shield, Target, Timer } from 'lucide-react';

interface EnhancedGameModesProps {
  currentMode: GameMode;
  onSelectMode: (mode: 'normal' | 'blitz' | 'bloodlust' | 'crazy' | 'survival' | 'timeAttack' | 'boss') => void;
  onClose: () => void;
}
...
export const EnhancedGameModes: React.FC<EnhancedGameModesProps> = ({ 
  ...
  const modes = [
    ...
    },
    {
      ...
      name: 'Survival',
      description: 'Limited lives - how far can you go?',
      icon: Shield,
      color: 'green',
      features: ['3 lives total', 'No health restoration', 'Increasing difficulty', 'Bonus rewards for distance']
    },
    {
      ...
      icon: Timer,
      color: 'orange',
      features: ['60 second timer', 'Rapid-fire questions', 'Score multipliers', 'Leaderboard competition']
    },
    {
      id: 'boss' as const,
      name: 'Boss Rush',
      description: 'Face powerful boss enemies with special abilities',
      icon: Target,
      color: 'pink',
      features: ['Boss enemies only', 'Special abilities', 'Unique mechanics', 'Epic rewards']
    }
  ];
  ...
  const getColorClasses = (color: string, isSelected: boolean) => {
    const colors = {
      ...
      orange: {
        ...
        bg: isSelected ? 'bg-orange-900/50' : 'bg-orange-900/20',
        text: 'text-orange-400',
        button: 'bg-orange-600 hover:bg-orange-500'
      },
      pink: {
        border: isSelected ? 'border-pink-500' : 'border-pink-500/30',
        bg: isSelected ? 'bg-pink-900/50' : 'bg-pink-900/20',
        text: 'text-pink-400',
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-gradient-to-br from-gray-900 to-slate-900 p-4 sm:p-6 rounded-lg border border-gray-500/50 max-w-6xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-white font-bold text-lg sm:text-xl">Enhanced Game Modes</h2>
        ...
        <div className="mt-6 p-3 sm:p-4 bg-gray-800/50 border border-gray-600/50 rounded-lg">
          ...
          {currentMode.current === 'timeAttack' && (
            <p className="text-center text-sm text-orange-400 mt-1">
              Best score: {currentMode.timeAttackScore} | Time left: {currentMode.timeAttackTimeLeft}s
            </p>
          )}
        </div>
      </div>
